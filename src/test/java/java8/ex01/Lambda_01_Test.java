...
public class Lambda_01_Test
{
    ...
    private List<Person> filter(List<Person> persons, PersonPredicate predicate) {
    	...
    	List<Person> sortie = new ArrayList<>();
    	
    	for (Person indv:persons)
    	{
    		if (predicate.test(indv))
    		{
    			sortie.add(indv);
    		}
    	}
    ...
    public void test_filter_by_age() throws Exception
    {
        ...
        {
        	//on cré ici une classe inconnu qui permet de redefinir directement l'interface sans
        	//passer par une classe intermediaire (qui ne sera sans doute pas réutiliser
        	@Override
    ...
    public void test_filter_by_password() throws Exception
    {
        ...
        PersonPredicate predicate = new PersonPredicate()
        {
        	...
        	public boolean test(Person p)
        	{
        		...
        		if (p.getAge()>49)
        		{
        			return ( DigestUtils.sha512Hex(p.getPassword()).equals(passwordSha512Hex) );
        		}
        		else
        		{return false;}
        	}
